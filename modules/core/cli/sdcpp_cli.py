...
from modules.shared_instance import SD_CLI, config, subprocess_manager
...
class CommandRunner(CommonRunner):
    ...
    def _get_next_synced_index(self) -> int:
        ...
        for d in dirs:
            ...
            for f in os.listdir(d):
                if (
                    f.startswith("pprompt_")
                    or f.startswith("nprompt_")
                    or f.startswith("command_")
                ) and f.endswith(".txt"):
                    try:
    ...
    def _set_output_path(self, dir_key: str, subctrl_id: int, extension: str):
        ...
        self.output_path = self._make_relative(
            generate_output_filename(
                output_dir, output_scheme, extension, name_parts, subctrl_id
            )
        )
    ...
    def _add_base_args(self):
        ...
        self.command.extend(
            [
                "--sampling-method",
                str(self._get_param("in_sampling")),
                "--steps",
                str(self._get_param("in_steps")),
                "-W",
                str(self._get_param("in_width")),
                "-H",
                str(self._get_param("in_height")),
                "--cfg-scale",
                str(self._get_param("in_cfg")),
                "-s",
                str(self._get_param("in_seed")),
                "-o",
                self.output_path,
        ...
        self.command.extend(
            [
                "--embd-dir",
                self._make_relative(config.get("emb_dir")),
            ]
        )

        rng = str(self._get_param("in_rng"))
        if rng and str(rng) != "Default":
            self.command.extend(["--rng", str(self._get_param("in_rng"))])
        ...
        if sampler_rng and str(sampler_rng) != "Default":
            self.command.extend(
                [
                    "--sampler-rng",
                    str(self._get_param("in_sampler_rng")),
                ]
            )
        ...
        if re.search(r"<lora:[^:]+:[^>]+>", f"{pp_text} {np_text}"):
            self.command.extend(
                [
                    "--lora-model-dir",
                    self._make_relative(config.get("lora_dir")),
                    "--lora-apply-mode",
                    str(self._get_param("in_lora_apply")),
                ]
            )
...
class ImageGenerationRunner(CommandRunner):
    ...
    def _get_model_options(self) -> dict[str, Any]:
        ...
        if diffusion_mode == DiffusionMode.CHECKPOINT:
            ...
            options["--diffusion-model"] = self._make_relative(
                self._get_param("f_unet_model")
            )
            options["--vae"] = self._make_relative(self._get_param("f_unet_vae"))
            options["--uncond-diffusion-model"] = self._make_relative(
                self._get_param("f_uncond_unet_model")
            )
            ...
            options["--llm_vision"] = self._make_relative(
                self._get_param("f_llm_vision")
            )
    ...
    def build_command(self, output_dir_key: str, subctrl_id: int):
        ...
        options = {
            ...
            **self._get_model_options(),
            ...
            # Scheduler
            "--scheduler": (
                self._get_param("in_scheduler")
                if not self._get_param("in_sigmas")
                else None
            ),
            "--sigmas": (
                self._get_param("in_sigmas")
            ...
            **(
                {
                    ...
                    "--pm-id-images-dir": self._make_relative(
                        self._get_param("in_phtmkr_id")
                    ),
                    "--pm-id-embed-path": self._make_relative(
                        self._get_param("in_phtmkr_emb")
                    ),
            ...
            **(
                {
                    ...
                    "--control-image": self._make_relative(
                        self._get_param("in_control_img")
                    ),
            ...
            # Chroma
            "--chroma-t5-mask-pad": (
                self._get_param("in_t5_mask_pad")
            ...
            **(
                {
                    ...
                    "--skip-layers": (
                        self._get_param("in_skip_layers")
                        if self._get_param("in_skip_layers") != ""
                        else None
                    ),
                }
                if self._get_param("in_slg_bool")
                else {}
            ),
            # Performance
            "--max-vram": (
                self._get_param("in_max_vram")
            ...
            **(
                {
                    ...
                    "--vae-relative-tile-size": (
                        f"{size}x{size}"
                        if (
                            self._get_param("in_vae_relative_bool")
                            and (size := self._get_param("in_vae_relative_tile_size"))
                        )
                        else None
                    ),
                }
            ...
            **(
                {
                    ...
                    "--cache-option": (
                        val.strip('"')
                        if (val := self._get_param("in_cache_option"))
                        and str(val).strip('"') != ""
                        else None
                    ),
                    "--scm-mask": (
                        self._get_param("in_scm_mask")
                        if self._get_param("in_scm_mask") != ""
                        else None
                    ),
                    "--scm-policy": (
                        self._get_param("in_scm_policy")
        ...
        flags.update(
            {
                "--taesd-preview-only": (
                    self._get_param("in_preview_taesd")
                    if self._get_param("in_preview_bool")
                    else False
                ),
                "--preview-noisy": (
                    self._get_param("in_preview_noisy")
                    if self._get_param("in_preview_bool")
                    else False
                ),
...
class Img2ImgRunner(ImageGenerationRunner):
    ...
    def build_command(self):
        ...
        self.command.extend(
            ["--init-img", self._make_relative(str(self._get_param("in_img_inp")))]
        )
        self.command.extend(["--strength", str(self._get_param("in_strength"))])
...
class ImgEditRunner(ImageGenerationRunner):
    ...
    def build_command(self):
        ...
        for img in ref_imgs:
            ...
            self.command.extend(["--ref-image", self._make_relative(img_path)])
...
class Any2VideoRunner(CommandRunner):
    ...
    def _add_base_args(self):
        ...
        self.command.extend(
            [
                "--video-frames",
                str(self._get_param("in_frames")),
                "--fps",
                str(self._get_param("in_fps")),
            ]
        )
    ...
    def build_command(self):
        ...
        options = {
            ...
            "--embeddings-connectors": self._make_relative(
                self._get_param("f_emb_connect")
            ),
            # Wan2.2 High Noise Configuration
            "--high-noise-diffusion-model": (
                self._make_relative(high_noise_model) if use_high_noise else None
            ),
            "--high-noise-cfg-scale": (
                self._get_param("in_high_noise_cfg") if use_high_noise else None
            ),
            "--high-noise-sampling-method": (
                self._get_param("in_high_noise_sampling") if use_high_noise else None
            ),
            "--high-noise-steps": (
                self._get_param("in_high_noise_steps") if use_high_noise else None
            ),
            "--high-noise-img-cfg-scale": (
                self._get_param("in_high_noise_img_cfg") if use_high_noise else None
            ),
            "--high-noise-guidance": (
                self._get_param("in_high_noise_guidance") if use_high_noise else None
            ),
            "--high-noise-slg-scale": (
                self._get_param("in_high_noise_slg_scale") if use_high_noise else None
            ),
            ...
            "--high-noise-eta": (
                self._get_param("in_high_noise_eta") if use_high_noise else None
            ),
            "--high-noise-skip-layers": (
                self._get_param("in_high_noise_skip_layers") if use_high_noise else None
            ),
            # Skip Layer Guidance (SLG)
            **(
                {
            ...
            # Wan & MoE Specifics
            "--moe-boundary": (
                self._get_param("in_moe_boundary")
                if self._get_param("in_moe_boundary_bool")
                else None
            ),
            "--vace-strength": (
                self._get_param("in_vace_strength")
            ...
            "--control-video": self._make_relative(
                self._get_param("in_control_video_dir")
            ),
            # TAESD
            "--taesd": self._get_param("f_taesd"),
            # Upscaling
            "--upscale-model": (
                self._get_param("f_upscl") if self._get_param("in_upscl_bool") else None
            ),
            "--upscale-repeats": (
                self._get_param("in_upscl_rep")
                if self._get_param("in_upscl_bool")
                else None
            ),
            "--upscale-tile-size": (
                self._get_param("in_upscl_tile_size")
                if self._get_param("in_upscl_bool")
                else None
            ),
            # Additional Params
            ...
            # ControlNet
            "--control-net": (
                self._get_param("f_cnnet") if self._get_param("in_cnnet_bool") else None
            ),
            "--control-image": (
                self._make_relative(self._get_param("in_control_img"))
                if self._get_param("in_cnnet_bool")
                else None
            ),
            "--control-strength": (
                self._get_param("in_control_strength")
                if self._get_param("in_cnnet_bool")
                else None
            ),
            "--prediction": (
...
def convert(params: dict):
    ...
    orig_model_path = os.path.relpath(
        os.path.join(in_model_dir, in_orig_model), start=exe_dir
    )
    ...
    if in_gguf_name:
        ...
        gguf_path = os.path.relpath(
            os.path.join(in_model_dir, in_gguf_name), start=exe_dir
        )
    else:
        model_name, _ = os.path.splitext(in_orig_model)
        gguf_path = os.path.relpath(
            os.path.join(in_model_dir, f"{model_name}-{in_quant_type}.gguf"),
            start=exe_dir,
        )
