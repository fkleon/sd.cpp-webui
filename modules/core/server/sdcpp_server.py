...
from modules.utils.metadata_utils import build_a1111_metadata, save_image_with_metadata
...
class ApiTaskRunner:
    ...
    def _extract_loras(self, text: str) -> tuple:
        ...
        def match_handler(match):
            ...
            extracted_loras.append(
                {"path": parsed_name, "multiplier": multiplier, "is_high_noise": False}
            )
            return ""
    ...
    def _get_extra_args_string(self) -> str:
        ...
        return (
            f"<sd_cpp_extra_args>{json.dumps(extra_args)}</sd_cpp_extra_args>"
            if extra_args
            else ""
        )
    ...
    def _process_response(self, data: dict):
        ...
        images_list = data.get("images", []) or [
            item.get("b64_json") for item in data.get("data", []) if "b64_json" in item
        ]
    ...
    def run(self) -> Generator:
            ...
            for key in ["init_images", "mask"]:
                if key in display_payload:
                    if isinstance(display_payload[key], list):
                        display_payload[key] = [
                            img[:60] + "..." for img in display_payload[key]
                        ]
        ...
        yield (self.fcommand, gr.skip(), gr.skip(), gr.skip(), None)

        try:
            if isinstance(payload_or_files, tuple):
                data, files = payload_or_files
                response = httpx.post(self.url, data=data, files=files, timeout=None)
            else:
                response = httpx.post(self.url, json=payload_or_files, timeout=None)
...
class Img2ImgApiRunner(ApiTaskRunner):
    def prepare(self):
        self._set_output_path(dir_key="img2img_dir", subctrl_id=0, extension="png")
