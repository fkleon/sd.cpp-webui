...
from modules.shared_instance import model_state, sd_options
...
def bind_generation_pipeline(api_func, ordered_keys, ordered_components, outputs_map):
    ...
    def poll_status():
        ...
        if not just_finished and (
            (owner != tab_id and not is_running) or (not is_running and q_len == 0)
        ):
            return (
                gr.skip(),
                gr.skip(),
        ...
        if q_len > 0:
            queue_display = gr.update(visible=True, value=f"⏳ Jobs in queue: {q_len}")
        else:
        ...
        return (cmd, prog_update, stat_update, stats, imgs, timer_update, queue_display)
    ...
    ).then(fn=lambda: gr.update(active=True), inputs=[], outputs=[outputs_map["timer"]])
...
def apply_lora(lora_model, lora_strength, lora_prompt_switch, pprompt, nprompt):
    if lora_model:
...
def unet_tab_switch(*args):
    ...
    return (
        ...
        gr.update(
            value=model_state.bak_flow_shift_bool, visible=True
        ),  # + flow_shift_bool
