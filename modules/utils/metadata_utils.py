...
def parse_png_metadata(img_path: str) -> str | None:
        ...
        with open(img_path, "rb") as file:
            ...
            while True:
                ...
                if chunk_type == "tEXt":
                    ...
                    return (
                        f"PNG: tEXt\n{keyword.decode('utf-8')}: {value.decode('utf-8')}"
                    )
...
def parse_jpg_metadata(img_path: str) -> str | None:
...
def parse_comfyui_workflow(text_data: str) -> dict[str, Any] | None:
    """
...
def parse_a1111_text(text_data: str) -> dict[str, Any]:
    ...
    sampler_match = re.search(r"Sampler:\s*([^,]+)", text_data, re.IGNORECASE)
    if sampler_match:
