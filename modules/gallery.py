...
class GalleryManager:
    ...
    def __init__(
        ...
        self.selected_media_index_on_page: int | None = None
        self.selected_media_global_index: int | None = None
    ...
    def _get_sorted_files(self) -> list[str]:
        ...
        try:
            files = (
                ...
                if f.lower().endswith(
                    (".png", ".jpg", ".jpeg", ".avi", ".webm", ".webp", ".mp4")
                )
            )
            if self.sort_order == "Date (Newest First)":
                ...
                return sorted(
                    files, key=lambda x: os.path.basename(x).lower(), reverse=True
                )
            else:
    ...
    def reload_gallery(
        ...
        return (
            gr.Gallery(
                value=page_files, label=current_label, selected_index=selected_index
            ),
            self.page_num,
        )
    ...
    def _navigate_page(self, direction: int) -> tuple[gr.Gallery, int]:
        ...
        if direction == 1:  # Next
            self.page_num = self.page_num + 1 if self.page_num < total_pages else 1
        elif direction == -1:  # Previous
            self.page_num = self.page_num - 1 if self.page_num > 1 else total_pages
    ...
    def delete_media(self) -> tuple[Any, ...]:
        ...
        if (
            not self.current_media_path
            or not os.path.exists(self.current_media_path)
            or self.selected_media_global_index is None
        ):
            gallery_update, page_num = self.reload_gallery(page_num=self.page_num)
