...
from modules.loader import get_loras, get_models, reload_models
from modules.shared_instance import config
from modules.utils.ui_state import get_session_value, update_session_cache
...
def create_model_widget(label, dir_key, option_key, **kwargs):
    """
    ...
    with gr.Group():
        ...
        with gr.Row():
            reload_btn = gr.Button(value=RELOAD_SYMBOL, scale=1)
            if dir_key == "lora_dir":
                reload_btn.click(
                ...
                reload_btn.click(
                    fn=reload_models, inputs=[path_component_txt], outputs=[dropdown]
                )
            clear_btn = gr.ClearButton(dropdown, scale=1)
            if option_key:
                clear_btn.click(
...
def create_unet_model_sel_ui():
    ...
    with gr.Row():
        gr.Markdown(
            "Supports: SD3, SD3.5, FLUX.1-Krea-dev, FLUX.1-dev, FLUX.1-schnell, FLUX.2-dev, Chroma, Qwen Image, Z-Image-Turbo, Anima, Ernie Image, Ideogram 4"
        )
    with gr.Row():
        with gr.Column():
...
def create_imgedit_model_sel_ui():
    ...
    with gr.Row():
        gr.Markdown(
            "Supports: FLUX.1-Kontext-dev, Qwen Image Edit, Qwen Image Edit 2509"
        )
    with gr.Row():
        with gr.Column():
            unet_model = create_model_widget(
...
def create_video_model_sel_ui():
    ...
    with gr.Row(), gr.Accordion(label="High Noise", open=False):
