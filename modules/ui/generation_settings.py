...
from .constants import QUANTS, RANDOM_SYMBOL, SAMPLERS, SCHEDULERS, SWITCH_V_SYMBOL


def create_quant_ui():
    """Create the model type selection UI"""
    with gr.Accordion(label="Quantization", open=False):
        with gr.Row():
        ...
        with gr.Row(), gr.Accordion(label="Tensor type rules", open=False):
    ...
    return {"in_model_type": model_type, "in_tensor_type_rules": tensor_type_rules}
...
def create_generation_settings_ui(unet_mode: bool = False):
    ...
    with gr.Row():
        with gr.Column(), gr.Group():
            ...
            switch_size = gr.Button(value=SWITCH_V_SYMBOL, scale=1)
            switch_size.click(
                switch_sizes, inputs=[height, width], outputs=[height, width]
            )
        with gr.Column():
            with gr.Row():
                steps = gr.Slider(
    ...
    with gr.Row():
        flow_shift_bool = gr.Checkbox(
            label="Enable Flow Shift", value=False, visible=unet_mode
        )
    ...
    with gr.Row():
        guidance_bool = gr.Checkbox(
            label="Enable distilled guidance", value=False, visible=unet_mode
        )
        guidance = gr.Slider(
            label="Guidance",
            minimum=0,
...
def create_bottom_generation_settings_ui():
    ...
    random_seed_btn.click(random_seed, inputs=[], outputs=[seed])

    return {"in_seed": seed, "in_clip_skip": clip_skip, "in_batch_count": batch_count}
