...
def create_lora_sel_ui():
    ...
    with gr.Row(), gr.Accordion(label="LoRAs selector", open=False):
        lora_model = create_model_widget(
            label="LoRA", dir_key="lora_dir", option_key=""
        )
        lora_prompt_switch = gr.Radio(
            label="Prompt", choices=["Positive", "Negative"], value="Positive"
        )
        ...
        apply_lora_btn = gr.Button(value="Apply to prompt", variant="primary")
...
def bind_lora_events(lora_ui, prompts_ui):
    ...
    lora_ui["in_apply_lora_btn"].click(
        apply_lora,
        inputs=[
            lora_ui["in_lora_model"],
            lora_ui["in_lora_strength"],
            lora_ui["in_lora_prompt_switch"],
            prompts_ui["in_pprompt"],
            prompts_ui["in_nprompt"],
        ],
        outputs=[prompts_ui["in_pprompt"], prompts_ui["in_nprompt"]],
    )
