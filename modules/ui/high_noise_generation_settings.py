...
def create_high_noise_generation_settings_ui():
    ...
    with gr.Row():
        high_noise_steps = gr.Slider(
            label="High Noise Steps", minimum=1, value=8, step=1, interactive=False
        )
    ...
    with gr.Row():
        ...
        high_noise_slg_scale = gr.Number(
            label="High Noise SLG Scale", value=0.0, step=0.1, interactive=False
        )
    with gr.Row():
        high_noise_skip_layer_start = gr.Number(
            label="High Noise SLG Start", value=0.01, step=0.01, interactive=False
        )
        high_noise_skip_layer_end = gr.Number(
            label="High Noise SLG End", value=0.2, step=0.01, interactive=False
        )
    with gr.Row():
        ...
        high_noise_eta = gr.Number(
            label="High Noise ETA", value=0.0, step=0.1, interactive=False
        )
    ...
    high_noise_comp = [
        ...
        high_noise_skip_layer_start,
        high_noise_skip_layer_end,
