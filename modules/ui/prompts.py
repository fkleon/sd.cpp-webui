...
def create_prompts_ui(nprompt_support=True):
    ...
    with gr.Row(), gr.Accordion(label="Saved prompts", open=False):
        with gr.Group():
            with gr.Column():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    del_prompt_btn = gr.Button(
                        value="Delete prompt", size="lg", variant="stop"
                    )
        with gr.Group():
            with gr.Column():
                new_prompt = gr.Textbox(
                    label="New Prompt name", placeholder="Prompt preset name"
                )
            with gr.Column():
    ...
    del_prompt_btn.click(
        delete_and_refresh_prompts, inputs=[saved_prompts], outputs=[saved_prompts]
    )

    reload_prompts_btn.click(refresh_prompt_list, inputs=[], outputs=[saved_prompts])

    load_prompt_btn.click(
        prompt_manager.get_prompt, inputs=[saved_prompts], outputs=[pprompt, nprompt]
    )
