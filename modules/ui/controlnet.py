...
def create_cnnet_ui():
    ...
    with gr.Accordion(label="ControlNet", open=False):
        cnnet_bool = gr.Checkbox(label="Enable ControlNet", value=False)
        with gr.Group():
            cnnet = gr.Dropdown(
                label="ControlNet",
            ...
            with gr.Row():
                reload_cnnet_btn = gr.Button(value=RELOAD_SYMBOL, interactive=False)
                clear_cnnet_btn = gr.ClearButton(cnnet, interactive=False)
        control_img = gr.Image(sources="upload", type="filepath", interactive=False)
        ...
        canny = gr.Checkbox(label="Canny (edge detection)", interactive=False)
    ...
    reload_cnnet_btn.click(reload_models, inputs=[cnnet_dir_txt], outputs=[cnnet])
