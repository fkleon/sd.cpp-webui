...
def create_photomaker_ui():
    ...
    with gr.Accordion(label="PhotoMaker settings", open=False), gr.Row(), gr.Group():
        with gr.Row():
            phtmkr_bool = gr.Checkbox(label="Enable PhotoMaker", value=False)
        with gr.Row():
            phtmkr = gr.Dropdown(
                label="PhotoMaker",
        ...
        with gr.Row():
            reload_phtmkr_btn = gr.Button(value=RELOAD_SYMBOL, interactive=False)
            clear_phtmkr_btn = gr.ClearButton(phtmkr, interactive=False)
        with gr.Row():
        ...
        with gr.Row():
            phtmkr_emb = gr.Textbox(
                label="PhotoMaker v2 id embed", value="", interactive=False
            )
        with gr.Row():
    ...
    reload_phtmkr_btn.click(reload_models, inputs=[phtmkr_dir_txt], outputs=[phtmkr])
