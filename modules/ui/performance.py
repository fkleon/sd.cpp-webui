    ...
    with gr.Accordion(label="Performance", open=False):
        ...
        with gr.Group():
            ...
            stream_layers = gr.Checkbox(
                label="Stream layers", value=config.get("def_stream_layers")
            )
            eager_load = gr.Checkbox(
                label="Eager load", value=config.get("def_eager_load")
            )
        ...
        with gr.Group():
            ...
            vae_conv_direct = gr.Checkbox(
                label="Conv2D Direct for VAE", value=config.get("def_vae_conv_direct")
            )
