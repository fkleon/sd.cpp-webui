...
def create_presets_ui():
    ...
    with gr.Row(), gr.Accordion(label="Saved presets", open=False):
        with gr.Group():
            with gr.Column():
            ...
            with gr.Column():
                with gr.Row():
                    load_preset_btn = gr.Button(value="Load preset", size="lg")
                    reload_presets_btn = gr.Button(value=RELOAD_SYMBOL)
                with gr.Row():
                    del_preset_btn = gr.Button(
                        value="Delete preset", size="lg", variant="stop"
                    )
        with gr.Group():
            with gr.Column():
                new_preset = gr.Textbox(
                    label="New Preset name", placeholder="Preset name"
                )
            with gr.Column():
                save_preset_btn = gr.Button(value="Save preset", size="lg")
...
def bind_presets_events(presets_ui, *ui_dicts, preset_flag=None):
    ...
    def save_and_refresh_presets(name, *values):
        if getattr(preset_manager, "is_default", lambda x: False)(name):
            gr.Warning(
                f"Cannot overwrite the default preset: '{name}'. Please use a different name."
            )
            return gr.skip()
