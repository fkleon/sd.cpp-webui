...
from modules.core.server.manager import start_server, stop_server
...
img2img_server_params: dict[str, gr.components.Component] = {}
server_process = None
...
with gr.Blocks() as img2img_server_block:
    ...
    with gr.Accordion(label="Server configuration", open=False):
        ...
        with gr.Accordion(label="Server Settings", open=False):
            ...
            listen_ip = gr.Textbox(
                label="Listen IP", show_label=True, value="127.0.0.1", interactive=True
            )
    ...
    with gr.Group(), gr.Row():
        with gr.Column(scale=1):
            with gr.Row():
                server_start = gr.Button(value="Start server", variant="primary")
            with gr.Row():
                server_stop = gr.Button(value="Stop server", variant="stop")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Tab("Generation Settings"):
                ...
                with gr.Row():
                    img_cfg_bool = gr.Checkbox(label="Enable Image CFG", value=False)
                    img_cfg = gr.Slider(
            ...
            with gr.Group():
                ...
                with gr.Row():
                    progress_textbox = gr.Textbox(
                        label="Progress:", visible=False, interactive=False
                    )
                with gr.Row():
    ...
    server_stop.click(
        fn=stop_server, inputs=[], outputs=[server_status, server_status_timer, gen_btn]
    )
    ...
    bind_generation_pipeline(img2img_api, ordered_keys, ordered_components, ui_outputs)
