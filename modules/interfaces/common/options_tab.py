...
from modules.ui.constants import SORT_OPTIONS, THEMES
...
with gr.Blocks() as options_block:
    ...
    with gr.Tab(label="Default models"):
        ...
        with gr.Row():
            registry.register(
                "def_ckpt",
                create_model_widget(
                    label="Checkpoint Model", dir_key="ckpt_dir", option_key="def_ckpt"
                ),
            )
            registry.register(
                "def_ckpt_vae",
                create_model_widget(
                    label="Checkpoint VAE", dir_key="vae_dir", option_key="def_ckpt_vae"
                ),
            )
        with gr.Row():
            registry.register(
                "def_unet",
                create_model_widget(
                    label="UNET Model", dir_key="unet_dir", option_key="def_unet"
                ),
            )
            registry.register(
                "def_unet_vae",
                create_model_widget(
                    label="UNET VAE", dir_key="vae_dir", option_key="def_unet_vae"
                ),
            )
        with gr.Row():
            registry.register(
                "def_clip_g",
                create_model_widget(
                    label="clip_g", dir_key="txt_enc_dir", option_key="def_clip_g"
                ),
            )
            registry.register(
                "def_clip_l",
                create_model_widget(
                    label="clip_l", dir_key="txt_enc_dir", option_key="def_clip_l"
                ),
            )
            registry.register(
                "def_clip_vision_h",
        ...
        with gr.Row():
            registry.register(
                "def_t5xxl",
                create_model_widget(
                    label="t5xxl", dir_key="txt_enc_dir", option_key="def_t5xxl"
                ),
            )
            registry.register(
                "def_umt5_xxl",
                create_model_widget(
                    label="umt5_xxl", dir_key="txt_enc_dir", option_key="def_umt5_xxl"
                ),
            )
        with gr.Row():
            registry.register(
                "def_llm",
                create_model_widget(
                    label="llm", dir_key="txt_enc_dir", option_key="def_llm"
                ),
            )
    ...
    with gr.Tab(label="Generation settings"):
        ...
        registry.register("def_sampling", generation_settings_ui["in_sampling"])
        registry.register("def_steps", generation_settings_ui["in_steps"])
        registry.register("def_scheduler", generation_settings_ui["in_scheduler"])
        registry.register("def_width", generation_settings_ui["in_width"])
        registry.register("def_height", generation_settings_ui["in_height"])
        registry.register("def_cfg", generation_settings_ui["in_cfg"])
        registry.register(
            "def_guidance_bool", generation_settings_ui["in_guidance_bool"]
        )
        registry.register("def_guidance", generation_settings_ui["in_guidance"])
        registry.register(
            "def_flow_shift_bool", generation_settings_ui["in_flow_shift_bool"]
        )
        registry.register("def_flow_shift", generation_settings_ui["in_flow_shift"])

        bottom_generation_settings_ui = create_bottom_generation_settings_ui()
        registry.register("def_seed", bottom_generation_settings_ui["in_seed"])
        registry.register(
    ...
    with gr.Group():
        ...
        with gr.Row():
            set_btn = gr.Button(value="Set new options", variant="primary")
            restore_btn = gr.Button(value="Restore Defaults", variant="stop")
        with gr.Row():
            restart_btn = gr.Button(value="Restart server", variant="stop")

    ordered_keys = registry.ordered_keys
    ordered_components = registry.ordered_components

    set_btn.click(registry.save, inputs=ordered_components, outputs=[status_textbox])

    restore_btn.click(registry.reset, inputs=[], outputs=[status_textbox])
