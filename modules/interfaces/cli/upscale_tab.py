...
from modules.utils.image_utils import size_updater, switch_sizes
from modules.utils.ui_events import get_ordered_inputs, update_interactivity
...
with gr.Blocks() as upscale_block:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                img_inp_upscale = gr.Image(sources="upload", type="filepath")
            with gr.Group():
                upscl = gr.Dropdown(
                    label="Upscaler",
            ...
            with gr.Group():
                rescale_bool = gr.Checkbox(
                    label="Allow changing initial size", value=False
                )
                with gr.Row():
                ...
                switch_size_btn = gr.Button(
                    value=SWITCH_V_SYMBOL, scale=1, interactive=False
                )
                rescale_comp = [init_width, init_height, switch_size_btn]
            switch_size_btn.click(
                switch_sizes,
                inputs=[init_height, init_width],
                outputs=[init_height, init_width],
            )
            upscl_rep = gr.Slider(
                label="Upscaler repeats", minimum=1, maximum=5, value=1, step=1
            )
            ...
            with gr.Row(), gr.Accordion(label="Extra", open=False):
        ...
        with gr.Column(), gr.Group():
            with gr.Row():
                upscl_btn = gr.Button(value="Upscale", size="lg", variant="primary")
            ...
            with gr.Row():
                stats = gr.Textbox(
                    label="Statistics:", show_label=True, value="", interactive=False
                )
            with gr.Row():
                command = gr.Textbox(
    ...
    img_inp_upscale.change(
        size_updater, inputs=img_inp_upscale, outputs=[init_width, init_height]
    )
